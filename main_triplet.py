...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        # ```Latent Model```- for image comparison to pick an image discriminant
        latent_model = ResNet50(input_shape=input_shape)
        get_feature_layer = K.function([latent_model.layers[0].input] + [K.learning_phase()], [latent_model.layers[-2].output])

        feature_vec = get_feature_layer([x_train, 0])[0]

        positive_train = []
        negative_train = []
        # positive_train ; 같은 클래스 이미지 중 가장 l2 distance가 큰 것
        # negative_train ; 다른 클래스 이미지 중 가장 l2 distance가 작은 것

        for i in idx:
            candidates_id = idx[y_train == y_train[i]]
            candidates_vec = feature_vec[y_train == y_train[i]]
            dist_ls = []
        ...
        for epoch in range(nb_epoch):
            res = model.fit(x_train_tri, labels,
