    ...
    def infer(queries, db):
        ...
        for (i, query) in enumerate(queries):
            ...
            for (j, ref) in enumerate(references):
                ...
                hamming_dict[ref] = hamming2(query_feature[i], reference_feature[j])

            sorted_hamming = sorted(hamming_dict.items(), key=lambda x: x[1])
            sorted_hamming = [x[0] for x in sorted_hamming]

            retrieval_results[query] = sorted_hamming
        print('done')
...
if __name__ == '__main__':
    ...
    model = InceptionResNetV2(input_shape=input_shape,)
