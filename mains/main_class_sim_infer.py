...
def bind_model(model):
    ...
    def infer(queries, db):
        ...
        #     retrieve = references[reference_class == query_class[i]]
        #     retrieve = [k.split('/')[-1].split('.')[0] for k in retrieve]
        #     retrieval_results[query] = retrieve
        # print(retrieve)
        ...
        for (i, query) in enumerate(queries):
            ...
            re_ranked_list = []
            for ref in ranked_list:
                if ref in retrieve:
                    re_ranked_list.insert(0, ref)
                else :
                    re_ranked_list.append(ref)

            retrieval_results[query] = re_ranked_list
        print('done')
